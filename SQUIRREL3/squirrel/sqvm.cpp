...
bool SQVM::ToString(const SQObjectPtr &o,SQObjectPtr &res)
{
	...
	case OT_FLOAT:
		...
		scsprintf(_sp(len),len,_SC("%g"),_float(o));
		break;
	case OT_INTEGER:
		len = rsl(NUMBER_MAX_CHAR+1);
		scsprintf(_sp(len),len,_PRINT_INT_FMT,_integer(o));
		break;
	case OT_BOOL:
		len = rsl(6);
		scsprintf(_sp(len),len,_integer(o)?_SC("true"):_SC("false"));
		break;
	case OT_TABLE:
	case OT_USERDATA:
	case OT_INSTANCE:
	...
	default:
		...
		scsprintf(_sp(len),len,_SC("(%s : 0x%p)"),GetTypeName(o),(void*)_rawval(o));
	}
